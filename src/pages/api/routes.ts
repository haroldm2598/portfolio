...
const generateEmailContent = (data: EmailProps) => {
	...
	const htmlData = Object.entries(data as Entries<typeof data>).reduce(
		(str, [key, val]) => {
			return (str += `<h1 class='form-heading' align='left'>${CONTACT_MESSAGE_FIELDS[key]}</h1><p class='form-answer' align='left'>${val}</p>`);
		},
		''
	);
